function [ surface, interior ] = default_forcings ( surface, interior, MARBL_depth_unit, lciso_on )
...
interior.forcing = zeros( interior.forcing_cnt, size(interior.domain.zw,2));
...
dust_flux = dust_flux/1e4/31536000;
...
surface.forcing(1, 9) =        1.0; % Air pressure (atm)
...
% s = 0.3;
s = 0.6;     % "stretching factor" for  Francois advection
interior.forcing(3, idx) = sst +fp_zgrid(size(interior.domain.zw,2)-idx(1)+1,sst-botT,s);
