...
!   end if
!
! However above is mess, when "commands" here have different number of args or results.
! So (limited amount of) checking is done after we know what cmd is...
! 
! IMPORTANT !!!
!
...
! that variable is ptr A F90 ptr like "delta_z" is -NOT- same as mwPointer!
!
...
! FIXME: large values of MAX_TMP_CNT makes everything extremely slow. WHY?
!        because its tripping over "-fmax-stack-var-size" which then causes recursion...
...
! =============================================================================
    ...
    case ('set_depth')      ! kmt is level in domain with ocean bottom
      ...
      numRows = 1
      call mxCopyPtrToReal8(mxGetPr(prhs(2)), tmp_array, numRows) ! MUST read into a real
      kmt = INT(tmp_array(1))
...
!  k = len(msg)
      ...
      k = k+2+len(trim(marbl_instance%tracer_metadata(n)%units))+1+1 ! extra +1 for the length of F90 str
    ...
    case ('diag_sname')
      ...
      numRows = 1
      call mxCopyPtrToReal8(mxGetPr(prhs(3)), tmp_array, numRows) ! MUST read into a real
      n = INT(tmp_array(1))
      !  write(msg, "(3A,I0,A)") '  returning short name of ', trim(diag_name), '(',n,')\n'; call mexPrintf(msg)
! FIXME: should check if index is valid...
!       msg = trim(diag%diags (n)%short_name)
! FIXME: next line works but throws compile warning about ‘diag.diags.data’ may be used uninitialized' 
!        possibly because MARBL is called in two steps, iniitalze and then compute.
      write (msg,*)trim(diag%diags(n)%short_name), " (",trim(diag%diags(n)%units), ")";
! FIXME: why oh why does F90 not do right thing with this??? Return number of diags...
!  k = len(msg)
      k     = len(trim(diag%diags (n)%short_name))
      k = k+2+len(trim(diag%diags (n)%units))     +1+1 ! extra +1 for length of F90 str
      !  call mexPrintf(msg//'\n')
      ! Now usual gyrations to return a string to Matlab
      string_return(1) = mxCreateString(msg)
      call mxSetN( string_return(literalOne), literalOne* 1)
...
! write(msg, *) 'diag_sname: n', n, "\n"; call mexPrintf(msg)
    ...
    case ('log')
!     FIXME: does not work when called from Matlab, seems log gets rest after every command or something....
        call outputLog ()

    case DEFAULT
      write(msg, *) 'DRIVER ERROR: mex_marbl_driver: unknown command (aka marble_phase): ', marbl_phase
      call mexWarnMsgTxt(msg)
  end select
...
!     nlev = mxGetM(prhs(2))    ! M is 1 for vector
  nlev = INT(mxGetN(prhs(2)))   ! N is length of vector
...
! FIXME: do NOT, do NOT, use literals in calls to mex functions...
    ...
    tmp_array(1) = DBLE(tracer_cnt)
    call mxCopyReal8ToPtr(tmp_array, mxGetPr(plhs(2)), numCols)

    ! Return interior_tendency_forcings size
    plhs(3) = mxCreateDoubleMatrix(numRows, numCols, ComplexFlag)
    tmp_array(1) = DBLE(size(marbl_instance%interior_tendency_forcings,1))
    call mxCopyReal8ToPtr(tmp_array, mxGetPr(plhs(3)), numCols)

    ! Return interior_tendency_diags size
    plhs(4) = mxCreateDoubleMatrix(numRows, numCols, ComplexFlag)
    tmp_array(1) = DBLE(size(marbl_instance%interior_tendency_diags%diags))
    call mxCopyReal8ToPtr(tmp_array, mxGetPr(plhs(4)), numCols)

    ! Return surface_flux_forcings size
    plhs(5) = mxCreateDoubleMatrix(numRows, numCols, ComplexFlag)
    tmp_array(1) = DBLE(size(marbl_instance%surface_flux_forcings))
    call mxCopyReal8ToPtr(tmp_array, mxGetPr(plhs(5)), numCols)

    ! Return interior_tendency_diags size
    plhs(6) = mxCreateDoubleMatrix(numRows, numCols, ComplexFlag)
    tmp_array(1) = DBLE(size(marbl_instance%surface_flux_diags%diags))
    call mxCopyReal8ToPtr(tmp_array, mxGetPr(plhs(6)), numCols)
...
subroutine outputSfo ()
  ...
  k = 0
  do j=1,INT(numCols)  ! FORTRAN is column major, Matlab and C are row major
    k = k+1
...
subroutine outputTracer ( myTracer )
